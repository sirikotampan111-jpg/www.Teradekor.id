...
import { Check, Phone, MessageCircle, ArrowRight, Heart } from 'lucide-react';
import { motion } from 'framer-motion';
import { Button } from '@/components/ui/button';
import { Card } from '@/components/ui/card';
import FloatingWhatsApp from '@/components/layout/FloatingWhatsApp';

const logo = '/logo.jpg'; // Logo semua section
...
// Animated letters component (interaktif & responsive)
...
export default function Home() {
  return (
    <div className="relative min-h-screen font-sans">
      ...
      {/* Hero Section */}
      <section className="relative min-h-screen flex flex-col justify-center items-center text-center px-4 sm:px-6 lg:px-8 pt-20">
            ...
            <Card key={idx} className="bg-black/40 border-2 border-[#C9A24D]/20 p-6 hover:border-[#C9A24D] transition-all duration-300 transform hover:scale-105 hover:shadow-2xl group">
              <div className="flex flex-col items-center text-center space-y-4">
                <div className="w-16 h-16 rounded-full bg-black/30 flex items-center justify-center">
                  <Image src={adv.icon} alt={adv.title} width={48} height={48} className="rounded-full" />
                </div>
            ...
            <Card key={idx} className="bg-black/40 border-2 border-[#C9A24D]/20 p-8 hover:border-[#C9A24D] transition-all duration-300 transform hover:scale-105 hover:shadow-2xl group">
              <div className="flex flex-col items-center text-center space-y-4">
                <div className="w-16 h-16 rounded-full flex items-center justify-center">
                  <Image src={hl.icon} alt={hl.title} width={48} height={48} className="rounded-full" />
                </div>
